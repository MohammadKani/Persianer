...
argument-hint: >-
  A development task for the Persianer extension, e.g. "fix date conversion
  bug", "add new profile option", "update popup UI", "test options page form".
---
...
| Build | ignore build -- PowerShell (`build.ps1 -Mode dev|prod`) → `build-dev/` or `build-prod/` |
...
> The user keeps Chrome running with **Load unpacked → `project root folder/`**. Use this loop.
...
```
You should see the user's real Chrome tabs (not a headless browser). If you see only `about:blank` from a sandboxed instance, **launch Chrome with CDP** (see §9.1) or **ask the user** to attach MCP to their launched Chrome (see §9).
...
### 9.1 Launch Chrome with CDP for dev-mode extension testing

If Chrome is **not** already running with CDP (port 9222 not listening), launch it
with a **separate profile directory** to avoid the silent-drop problem on the
default profile:

```powershell
# Paths — adjust if Chrome is installed elsewhere on this machine
$chromePath = "C:\Program Files\Google\Chrome\Application\chrome.exe"
$extensionDir = "Z:\Dev\Persianer\"   # dev build output

# 1. Check if CDP port is already listening
...
if (-not $listening) {
  ...
  Start-Process $chromePath `
    -ArgumentList "--remote-debugging-port=9222 --user-data-dir=C:\chrome-debug"
...
# 2. Confirm DevTools Protocol responds
$version = Invoke-WebRequest http://127.0.0.1:9222/json/version -UseBasicParsing
Write-Host "✅ CDP reachable — $($version.Content)"
```

> ⚠️ **`--user-data-dir` is mandatory.** Chrome silently ignores
> `--remote-debugging-port` on the default profile. See
> `userMemory/chrome-cdp-debugging.md` for the full explanation.

After Chrome is up:
1. Navigate to `chrome://extensions/` via the browser page
2. Enable **Developer mode** (toggle top-right)
3. Click **Load unpacked** → select `Z:\Dev\Persianer\build-dev\`
4. The extension is now loaded in dev mode — rebuild + reload picks up changes

> 💡 Keep this Chrome instance running between edits. Closing it loses the
> loaded extension state and requires re-loading unpacked.
